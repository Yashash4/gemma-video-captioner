...
`gemma.py` exposes one interface (`call_vision` / `call_text`) over a pluggable provider chosen by the `PROVIDER` environment variable. The submitted image sets `PROVIDER=fireworks` and calls Gemma-4-31B served on Fireworks AI, a dedicated on-demand deployment reached through an OpenAI-compatible chat completions endpoint that accepts image inputs as base64 data URLs. On this path Gemma-4's thinking mode is explicitly disabled (`enable_thinking=False`, `reasoning_effort=none`): left on, the model emits its chain-of-thought into a separate reasoning field and returns an empty `content` on harder prompts, which would silently drop that caption to a fallback. Disabling it keeps `content` populated and is also markedly faster, since no tokens are spent reasoning. Two alternate providers are implemented in the same file: Google AI Studio (`google`, its `generateContent` endpoint) and Ollama Cloud (`ollama`, `gemma4:31b-cloud` with `think:false` and Gemma-4's calibrated sampling `top_k=64`/`top_p=0.95`). All providers that support it use Gemma-4's native `system` role for the persistent grounding rules; the Google path folds those rules into the prompt text because its Gemma endpoint rejects a system instruction.
